...
if BINARY_SUPPORT:
    from snn_classification_realtime.viz.loaders import (
        load_activity_dataset as _load_activity_dataset,
        group_by_image,
    )
else:
    _load_activity_dataset = None  # type: ignore


def load_activity_data(path: str):
    """Load binary activity dataset. Raises if BINARY_SUPPORT is False."""
    if not BINARY_SUPPORT:
        raise ImportError(
            "Binary dataset support not available. Install required dependencies."
        )
    assert _load_activity_dataset is not None
    return _load_activity_dataset(path)
...
_config = VizConfig()
PLOT_IMAGE_SCALE: float = _config.plot_image_scale
MATPLOTLIB_DPI: int = _config.matplotlib_dpi
CACHE_VERSION: str = _config.cache_version
...
def main():
    ...
    cache_dir = get_cache_dir(structured_output_dir, CACHE_VERSION)
